"""
pull_data.py — Pull BLS B-3a aggregate weekly payroll (AWP) data and CPI-U.

Run from the earnings/ directory:
    python pull_data.py

Outputs:
    b3a_series.csv     — metadata for all AWP series (series_id, industry_code, ...)
    b3a_mapping.csv    — b3a_series + display_level, benchmark_id, benchmark_name
    b3a_wide.csv       — wide-format AWP data (date index × series_id columns)
...
def fetch_ce_series_metadata() -> pd.DataFrame:
    """Download ce.series metadata file from BLS and filter for AWP series."""
    ...
    # Filter: data_type_code == 57 (AWP all employees), seasonal == S
    df57 = df[df["data_type_code"] == "57"]
    n_sa = int((df57["seasonal"] == "S").sum())
    n_nsa = int((df57["seasonal"] == "U").sum())
    print(f"  data_type 57 series: {n_sa} seasonally adjusted (S), {n_nsa} not seasonally adjusted (U)")
    if n_sa < 500:
        print(
            f"  WARNING: Only {n_sa} SA series found for data type 57 — "
            f"detailed industries may only be available NSA ({n_nsa} U series exist)",
            file=sys.stderr,
        )

    df = df57[df57["seasonal"] == "S"].copy()
    ...
    print(f"  Found {len(df)} AWP series (private, seasonally adjusted)")
...
def pull_all_series(all_series_ids: list) -> dict:
    """Batch-fetch all AWP series + CPI-U from BLS API v2."""
...
def main():
    ...
    awp_ids = mapping_df["series_id"].tolist()
    print(f"\nPulling {len(awp_ids)} AWP series + CPI-U from BLS API…")
    all_data = pull_all_series(awp_ids)
    print(f"\nReceived data for {len(all_data)} series")

    # 4. Save AWP wide CSV
    awp_df = build_wide_df(all_data, awp_ids)
    awp_df.to_csv(HERE / "b3a_wide.csv")
    print(f"Saved b3a_wide.csv ({len(awp_df)} rows × {len(awp_df.columns)} series)")
